use gridway_store::{KVStore, MemStore, StateManager};
...
fn main() {
    ...
    println!("\n4. Simulating transfer: cosmos1test sends 100 stake to cosmos1other...");
    {
        ...
        let test_stake = store
            .get(b"balance_cosmos1test_stake")
            .unwrap()
            .map(|v| String::from_utf8_lossy(&v).parse::<u64>().unwrap())
            .unwrap_or(0);
        let other_stake = store
            .get(b"balance_cosmos1other_stake")
            .unwrap()
            .map(|v| String::from_utf8_lossy(&v).parse::<u64>().unwrap())
            .unwrap_or(0);
