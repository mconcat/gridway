...
pub enum AbciError {
    ...
    BaseApp(#[from] gridway_baseapp::BaseAppError),
...
impl AbciService for AbciServer {
    ...
    async fn check_tx(
        ...
        let exec_mode = match req.r#type {
            1 => gridway_baseapp::ExecMode::ReCheck, // RECHECK = 1
            _ => gridway_baseapp::ExecMode::Check,   // NEW = 0 or any other value
        };
...
mod tests {
    ...
    async fn test_error_codes() {
        ...
        assert_eq!(
            AbciError::BaseApp(gridway_baseapp::BaseAppError::InvalidTx("test".to_string())).code(),
            100
        );
    }
}
