...
struct StoredKey {
    ...
    privkey: PrivateKey,
    #[zeroize(skip)]
    pubkey: gridway_crypto::PublicKey,
    #[zeroize(skip)]
    address: gridway_types::address::AccAddress,
}
...
impl Keyring for MemoryKeyring {
    ...
    async fn sign(&self, name: &str, data: &[u8]) -> Result<Vec<u8>, KeyringError> {
        ...
        use gridway_crypto::signature::sign_message;
...
impl OsKeyring {
    ...
    async fn list_keys_from_os(&self) -> Result<Vec<String>, KeyringError> {
        ...
        let registry_entry = keyring::Entry::new(&self.service_name, "_gridway_key_registry")
            .map_err(|e| {
                KeyringError::BackendError(format!("Failed to create registry entry:: {e}"))
            })?;

        println!("DEBUG: list_keys_from_os: attempting to read registry with service:: {} user: _gridway_key_registry", self.service_name);
    ...
    fn to_stored_key(&self, serializable: SerializableKey) -> Result<StoredKey, KeyringError> {
        ...
        let address = gridway_types::address::AccAddress::from_bech32(&serializable.address)
        ...
        for name in os_key_names {
            if let Ok(serializable) = self.retrieve_key_from_os(&name).await {
                keys.push(KeyInfo {
                    name: name.clone(),
                    pubkey: serializable.pubkey.clone(),
                    address: gridway_types::address::AccAddress::from_bech32(&serializable.address)
                        .map_err(|e| KeyringError::BackendError(format!("Invalid address:: {e}")))?
                        .1,
                });
            }
