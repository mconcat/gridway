...
mod tests {
    ...
    fn setup_test_context() -> (AbiContext, Arc<VirtualFilesystem>, Arc<CapabilityManager>) {
        ...
        vfs.mount_store("test_module".to_string(), test_store).unwrap();
        
        // Add VFS capabilities
        vfs.add_capability(crate::vfs::Capability::Read("test_module".to_string())).unwrap();
        vfs.add_capability(crate::vfs::Capability::Write("test_module".to_string())).unwrap();
        vfs.add_capability(crate::vfs::Capability::Create("test_module".to_string())).unwrap();
    ...
    fn test_host_state_get_set() {
        ...
        host_state_set_func.call(&mut store, &params, &mut results).unwrap();
        ...
        let mut results = vec![Val::I32(0)];
        host_state_get_func.call(&mut store, &params, &mut results).unwrap();
        ...
        let read_len = u32::from_le_bytes([len_buffer[0], len_buffer[1], len_buffer[2], len_buffer[3]]) as usize;
    ...
    fn test_host_capability_check() {
        ...
        // Get host function
        let host_capability_check = linker.get(&mut store, "env", "host_capability_check").unwrap();
        ...
        let mut results = vec![Val::I32(0)];
        host_capability_check_func.call(&mut store, &params, &mut results).unwrap();
        ...
        assert_eq!(results[0].unwrap_i32(), AbiResultCode::PermissionDenied as i32);
    }
    ...
    fn test_host_ipc_send() {
        ...
        let mut results = vec![Val::I32(0)];
        host_ipc_send_func.call(&mut store, &params, &mut results).unwrap();
