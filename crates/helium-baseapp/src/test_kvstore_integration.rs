...
mod tests {
    ...
    fn test_kvstore_prefix_access_control() {
        ...
        kvstore_host.register_component_prefix("ante-handler".to_string(), "/ante/".to_string()).unwrap();
        kvstore_host.register_component_prefix("begin-blocker".to_string(), "/begin/".to_string()).unwrap();
        kvstore_host.register_component_prefix("end-blocker".to_string(), "/end/".to_string()).unwrap();

        // Open store for ante-handler
        let ante_store = kvstore_host
            .open_store(&mut table, "ante-handler")
            .unwrap();

        // Should be able to read /ante/ keys (without the prefix in the key)
        {
            let ante_store_copy = wasmtime::component::Resource::<crate::kvstore_resource::KVStoreResource>::new_own(ante_store.rep());
            let store_resource = kvstore_host.get_resource(&mut table, ante_store_copy).unwrap();
            assert_eq!(
    ...
    fn test_kvstore_component_isolation() {
        ...
        {
            let begin_store_copy = wasmtime::component::Resource::<crate::kvstore_resource::KVStoreResource>::new_own(begin_store.rep());
            let store_resource = kvstore_host.get_resource(&mut table, begin_store_copy).unwrap();
        ...
        {
            let ante_store_copy = wasmtime::component::Resource::<crate::kvstore_resource::KVStoreResource>::new_own(ante_store.rep());
            let ante_resource = kvstore_host.get_resource(&mut table, ante_store_copy).unwrap();
            assert_eq!(
            ...
            let begin_store_copy = wasmtime::component::Resource::<crate::kvstore_resource::KVStoreResource>::new_own(begin_store.rep());
            let begin_resource = kvstore_host.get_resource(&mut table, begin_store_copy).unwrap();
            assert_eq!(
