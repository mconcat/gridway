...
mod tests {
    ...
    fn test_component_kvstore_isolation() {
        ...
        host.load_component("begin-blocker", &begin_bytes, begin_info).unwrap();
        ...
        host.load_component("end-blocker", &end_bytes, end_info).unwrap();

        // Pre-populate the store with data for each component
        {
            let mut store = base_store.lock().unwrap();
            // Data for begin-blocker
            store.set(b"/begin/proposer_address", b"\x01\x02\x03\x04").unwrap();
            // Data for end-blocker
            store.set(b"/end/inflation_rate", &0.05f64.to_le_bytes()).unwrap();
            store.set(b"/end/last_reward_height", &100u64.to_le_bytes()).unwrap();
            store.set(b"/end/total_power", &1000i64.to_le_bytes()).unwrap();
            store.set(b"/end/proposer_address", b"\x05\x06\x07\x08").unwrap();
        }

        // Execute begin-blocker
        let begin_result = host.execute_begin_blocker(
            1000,      // block_height
        ...
        // Execute end-blocker
        let end_result = host.execute_end_blocker(
            1000,      // block_height
            1234567890, // block_time
            "test-chain", // chain_id
            1_000_000,   // gas_limit
    ...
    fn test_component_kvstore_persistence() {
        ...
        let result1 = host.execute_begin_blocker(
            1000,      // block_height
            1234567890, // block_time
            "test-chain", // chain_id
            1_000_000,   // gas_limit
        ...
        let result2 = host.execute_begin_blocker(
            1001,      // block_height
    ...
    fn test_kvstore_prefix_enforcement() {
        ...
        kvstore_host.register_component_prefix(
        ...
        let store_handle = kvstore_host.open_store(&mut table, "custom-component").unwrap();
    ...
    fn test_kvstore_edge_cases() {
        ...
        let mut table = wasmtime_wasi::ResourceTable::new();
        let store_handle = kvstore_host.open_store(&mut table, "test-component").unwrap();
        let store = kvstore_host.get_resource(&mut table, store_handle).unwrap();
        ...
        assert_eq!(store.get(&large_key).unwrap(), Some(b"large_key_value".to_vec()));
    ...
    fn test_multiple_components_isolation() {
        ...
        let store_a_handle = kvstore_host.open_store(&mut table_a, "component-a").unwrap();
        let store_a = kvstore_host.get_resource(&mut table_a, store_a_handle).unwrap();

        let mut table_b = wasmtime_wasi::ResourceTable::new();
        let store_b_handle = kvstore_host.open_store(&mut table_b, "component-b").unwrap();
        let store_b = kvstore_host.get_resource(&mut table_b, store_b_handle).unwrap();
