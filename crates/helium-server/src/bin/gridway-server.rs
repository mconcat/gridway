//! Gridway Server Binary
//!
//! This is the main entry point for the Gridway blockchain node.
...
#[command(about = "Gridway blockchain node", long_about = None)]
struct Cli {
...
enum Commands {
    ...
    Init {
        ...
        #[arg(long, default_value = "gridway-testnet")]
        chain_id: String,
    },
    /// Start the node
    Start {
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    tracing_subscriber::fmt()
        .with_env_filter(
            tracing_subscriber::EnvFilter::from_default_env()
                .add_directive("gridway=info".parse().unwrap()),
        )
        .init();
    ...
    match cli.command {
        Commands::Init { chain_id } => {
            info!("Initializing Gridway node with chain ID: {}", chain_id);
        ...
        Commands::Start {
            ...
            let config = if config_path.exists() {
                ...
                AbciConfig {
                    ...
                    persist_interval: 1,
                    retain_blocks: 0,
                    chain_id: chain_id.unwrap_or_else(|| "gridway-testnet".to_string()),
                }
            };
        ...
        Commands::Version => {
            println!("Gridway v{}", env!("CARGO_PKG_VERSION"));
