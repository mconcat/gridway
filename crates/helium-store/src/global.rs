...
use crate::{KVStore, JMTStore, Result, StoreError};
...
impl GlobalAppStore {
    ...
    pub fn register_namespace(&self, namespace: &str, read_only: bool) -> Result<()> {
        ...
        if namespaces.contains_key(namespace) {
            return Err(StoreError::BackendError(format!("Namespace {} already registered", namespace)));
        }
    ...
    pub fn get_namespace(&self, namespace: &str) -> Result<NamespacedStore> {
        ...
        let config = namespaces.get(namespace)
...
impl NamespacedStore {
    ...
    fn strip_prefix(&self, key: &[u8]) -> Option<Vec<u8>> {
        ...
        if key.len() > prefix_len && key.starts_with(self.config.namespace.as_bytes()) && key[self.config.namespace.len()] == b'/' {
...
impl KVStore for NamespacedStore {
    ...
    fn set(&mut self, key: &[u8], value: &[u8]) -> Result<()> {
        if self.config.read_only {
            return Err(StoreError::WriteFailed("Namespace is read-only".to_string()));
        }
    ...
    fn prefix_iterator(&self, prefix: &[u8]) -> Box<dyn Iterator<Item = (Vec<u8>, Vec<u8>)> + '_> {
        ...
        let items: Vec<_> = store.prefix_iterator(&full_prefix)
            .filter_map(move |(k, v)| {
...
mod tests {
    ...
    fn test_namespace_isolation() {
        ...
        store.set_namespaced("auth", b"key1", b"auth_value").unwrap();
        store.set_namespaced("bank", b"key1", b"bank_value").unwrap();
    ...
    fn test_unregistered_namespace() {
        ...
        assert!(store.set_namespaced("unregistered", b"key", b"value").is_err());
    }
}