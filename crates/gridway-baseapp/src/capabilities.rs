    ...
    pub fn from_string(cap_str: &str) -> Result<Self> {
        ...
        if parts.is_empty() {
            return Err(CapabilityError::InvalidFormat(
                "empty capability string".to_string(),
            ));
        }
        ...
        match parts[0] {
            "read_state" => {
                if parts.len() != 2 {
                    return Err(CapabilityError::InvalidFormat(
                        "read_state requires namespace".to_string(),
                    ));
                }
            ...
            "write_state" => {
                if parts.len() != 2 {
                    return Err(CapabilityError::InvalidFormat(
                        "write_state requires namespace".to_string(),
                    ));
                }
            ...
            "delete_state" => {
                if parts.len() != 2 {
                    return Err(CapabilityError::InvalidFormat(
                        "delete_state requires namespace".to_string(),
                    ));
                }
            ...
            "list_state" => {
                if parts.len() != 2 {
                    return Err(CapabilityError::InvalidFormat(
                        "list_state requires namespace".to_string(),
                    ));
                }
            ...
            "send_msg" => {
                if parts.len() != 2 {
                    return Err(CapabilityError::InvalidFormat(
                        "send_msg requires message type".to_string(),
                    ));
                }
            ...
            "receive_msg" => {
                if parts.len() != 2 {
                    return Err(CapabilityError::InvalidFormat(
                        "receive_msg requires message type".to_string(),
                    ));
                }
            ...
            "emit_event" => {
                if parts.len() != 2 {
                    return Err(CapabilityError::InvalidFormat(
                        "emit_event requires event type".to_string(),
                    ));
                }
            ...
            "execute_module" => {
                if parts.len() != 2 {
                    return Err(CapabilityError::InvalidFormat(
                        "execute_module requires module name".to_string(),
                    ));
                }
            ...
            "allocate_memory" => {
                if parts.len() != 2 {
                    return Err(CapabilityError::InvalidFormat(
                        "allocate_memory requires size limit".to_string(),
                    ));
                }
                let size = parts[1].parse::<u64>().map_err(|_| {
                    CapabilityError::InvalidFormat("invalid memory size".to_string())
                })?;
            ...
            "crypto" => {
                if parts.len() != 2 {
                    return Err(CapabilityError::InvalidFormat(
                        "crypto requires operation".to_string(),
                    ));
                }
            ...
            "network" => {
                if parts.len() < 2 {
                    return Err(CapabilityError::InvalidFormat(
                        "network requires operation".to_string(),
                    ));
                }
                let net_cap = match parts[1] {
                    "http" => {
                        if parts.len() != 3 {
                            return Err(CapabilityError::InvalidFormat(
                                "network:http requires domain".to_string(),
                            ));
                        }
                    ...
                    "tcp_connect" => {
                        if parts.len() != 4 {
                            return Err(CapabilityError::InvalidFormat(
                                "network:tcp_connect requires host:port".to_string(),
                            ));
                        }
                        let port = parts[3].parse::<u16>().map_err(|_| {
                            CapabilityError::InvalidFormat("invalid port number".to_string())
                        })?;
                    ...
                    "tcp_listen" => {
                        if parts.len() != 3 {
                            return Err(CapabilityError::InvalidFormat(
                                "network:tcp_listen requires port".to_string(),
                            ));
                        }
            ...
            _ => {
                ...
                if parts.len() != 3 {
                    return Err(CapabilityError::InvalidFormat(
                        "custom capability requires namespace:operation".to_string(),
                    ));
                }
                Ok(CapabilityType::Custom(
                    parts[1].to_string(),
                    parts[2].to_string(),
                ))
            }
...
impl CapabilityManager {
    ...
    pub fn grant_capability(
        ...
        debug!(
            "Granting capability {:?} to module {} by {}",
            capability, module, granter
        );

        // Check if granter has the capability to grant
        if granter != "system"
            && !self.has_capability(granter, &CapabilityType::CreateCapability)?
        {
    ...
    pub fn revoke_capability(&self, module: &str, capability: &CapabilityType) -> Result<()> {
        debug!(
            "Revoking capability {:?} from module {}",
            capability, module
        );
        ...
        info!(
            "Revoked capability {} from module {}",
            capability.to_string(),
            module
        );
        Ok(())
    ...
    pub fn delegate_capability(
        ...
        debug!(
            "Delegating capability {:?} from {} to {}",
            capability, from_module, to_module
        );
    ...
    pub fn check_access(&self, module: &str, resource: &str, operation: &str) -> Result<()> {
        debug!(
            "Checking access for module {} to resource {} operation {}",
...
mod tests {
    ...
    fn test_capability_parsing() {
        ...
        assert!(
            matches!(cap, CapabilityType::SendMessage(msg) if msg == "/cosmos.bank.v1beta1.MsgSend")
        );
        ...
        assert!(matches!(
            cap,
            CapabilityType::Crypto(CryptoCapability::Sign)
        ));

        let cap = CapabilityType::from_string("network:http:api.example.com").unwrap();
        assert!(
            matches!(cap, CapabilityType::Network(NetworkCapability::HttpRequest(domain)) if domain == "api.example.com")
        );
    ...
    fn test_capability_manager() {
        ...
        manager
            .grant_capability(
                "module_a",
                CapabilityType::WriteState("auth".to_string()),
                "system",
                false,
            )
            .unwrap();
        ...
        // Check capabilities
        assert!(manager
            .has_capability("module_a", &CapabilityType::ReadState("auth".to_string()))
            .unwrap());
        assert!(manager
            .has_capability("module_a", &CapabilityType::WriteState("auth".to_string()))
            .unwrap());
        assert!(!manager
            .has_capability("module_a", &CapabilityType::ReadState("bank".to_string()))
            .unwrap());
        ...
        // Test revocation
        manager
            .revoke_capability("module_a", &CapabilityType::WriteState("auth".to_string()))
            .unwrap();
        assert!(!manager
            .has_capability("module_a", &CapabilityType::WriteState("auth".to_string()))
            .unwrap());
    ...
    fn test_capability_delegation() {
        ...
        manager
            .grant_capability(
                "module_a",
                CapabilityType::DelegateCapability,
                "system",
                false,
            )
            .unwrap();
        ...
        assert!(manager
            .has_capability("module_b", &CapabilityType::ReadState("auth".to_string()))
            .unwrap());
    ...
    fn test_access_control() {
        ...
        assert!(manager
            .check_access("bank_module", "bank", "delete")
            .is_err()); // No delete capability
    }
    ...
    fn test_default_capabilities() {
        ...
        assert!(manager
            .has_capability("new_module", &CapabilityType::SystemInfo)
            .unwrap());
        assert!(manager
            .has_capability(
                "new_module",
                &CapabilityType::AllocateMemory(16 * 1024 * 1024)
            )
            .unwrap());
