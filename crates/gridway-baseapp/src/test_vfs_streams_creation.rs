...
mod tests {
    ...
    use crate::vfs_wasi_impl::{DescriptorKind, VfsFilesystem};
    ...
    fn setup_test_vfs_with_file() -> (VfsFilesystem, u32) {
        ...
        vfs.lock()
            .unwrap()
            .mount_store("test".to_string(), store)
            .unwrap();

        // Add a test capability
        vfs.lock()
            .unwrap()
            .add_capability(Capability::Write("/test".into()))
            .unwrap();
        ...
        let vfs_fd = vfs
            .lock()
            .unwrap()
            .open(&std::path::Path::new("/test/file.txt"), true)
    ...
    fn setup_test_vfs_with_readonly_file() -> (VfsFilesystem, u32) {
        ...
        let vfs_fd = vfs
            .lock()
            .unwrap()
            .open(&std::path::Path::new("/test/readonly.txt"), false)
    ...
    fn test_write_stream_on_readonly_file_fails() {
        ...
        assert!(
            result.is_err(),
            "Should fail to create write stream on read-only file"
        );
    ...
    fn test_append_stream_on_readonly_file_fails() {
        ...
        assert!(
            result.is_err(),
            "Should fail to create append stream on read-only file"
        );
    }
    ...
    fn test_stream_on_directory_fails() {
        ...
        assert!(
            result.is_err(),
            "Should fail to create read stream on directory"
        );
        ...
        assert!(
            result.is_err(),
            "Should fail to create write stream on directory"
        );
        ...
        assert!(
            result.is_err(),
            "Should fail to create append stream on directory"
        );
    }
