...
fn main() {
    ...
    let all_modules_exist = ["ante_handler.wasm", "begin_blocker.wasm", "end_blocker.wasm", "tx_decoder.wasm"]
        .iter()
...
fn build_wasi_modules() {
    ...
    for (module_name, crate_name) in &wasi_modules {
        ...
        let module_path = workspace_root.join("crates").join("wasi-modules").join(module_name);
        ...
        let target_dir = workspace_root.join("target").join("wasm32-wasip1").join("release");
