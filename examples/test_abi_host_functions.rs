...
use helium_baseapp::{
    abi::{AbiContext, HostFunctions, AbiResultCode},
    vfs::{VirtualFilesystem, Capability as VfsCapability},
...
fn main() {
    ...
    vfs.mount_store("test_module".to_string(), test_store.clone()).unwrap();
    
    // Add VFS capabilities
    vfs.add_capability(VfsCapability::Read("test_module".to_string())).unwrap();
    vfs.add_capability(VfsCapability::Write("test_module".to_string())).unwrap();
    vfs.add_capability(VfsCapability::Create("test_module".to_string())).unwrap();
    ...
    let has_read = cap_manager.has_capability("test_module", &CapabilityType::ReadState("test_module".to_string())).unwrap();
    println!("✓ Module has read capability: {}", has_read);
    
    let has_forbidden = cap_manager.has_capability("test_module", &CapabilityType::WriteState("forbidden".to_string())).unwrap();
